...
real* hy_dens_cell;         //hydrostatic density (vert cell avgs).   Dimensions: (1-hs:nz+hs)
real* hy_dens_theta_cell;   //hydrostatic rho*t (vert cell avgs).     Dimensions: (1-hs:nz+hs)
real* hy_dens_int;          //hydrostatic density (vert cell interf). Dimensions: (1:nz+1)
real* hy_dens_theta_int;    //hydrostatic rho*t (vert cell interf).   Dimensions: (1:nz+1)
real* hy_pressure_int;      //hydrostatic press (vert cell interf).   Dimensions: (1:nz+1)
real* dev_state, * dev_state_tmp, * dev_flux, * dev_tend, * dev_hy_dens_cell, * dev_hy_dens_theta_cell;
real* dev_hy_dens_int, * dev_hy_dens_theta_int, * dev_hy_pressure_int;
...
real* tend;                 //Fluid state tendencies.  Dimensions: (nx,nz,NUM_VARS)
...
void   output(real* state, double etime);
void   ncwrap(int ierr, int line);
void   perform_timestep(real* state, real* state_tmp, real* flux, real* tend, real dt);
void   semi_discrete_step(real* state_init, real* state_forcing, real* state_out, real dt, int dir, real* flux, real* tend);
void   compute_tendencies_x(real* state, real* flux, real* tend, real dt);
void   compute_tendencies_z(real* state, real* flux, real* tend, real dt);
void   set_halo_values_x(real* state);
void   set_halo_values_z(real* state);
...
int main(int argc, char** argv) {
    ...
    while (etime < sim_time) {
        ...
        perform_timestep(dev_state, dev_state_tmp, flux, tend, dt);
        //Inform the user
#ifndef NO_INFORM
    ...
    if (mainproc) {
        std::cout << "CPU Time: " << std::chrono::duration<real>(t2 - t1).count() << " sec\n";
    }
...
void perform_timestep(real* state, real* state_tmp, real* flux, real* tend, real dt) {
    ...
    if (direction_switch) { direction_switch = 0; }
    else { direction_switch = 1; }
}

__global__ void semi_discrete_step_kernel(real* state_out, real* state_init, real* tend, real dt, int nx, int hs, int nz) {
...
void semi_discrete_step(real* state_init, real* state_forcing, real* state_out, real dt, int dir, real* flux, real* tend) {
    ...
    // cudaMemcpy(dev_state, state_init, (nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS * sizeof(real), cudaMemcpyHostToDevice);

    dim3 dimGrid(nz * nx / 8);
    dim3 dimBlock(32);

    semi_discrete_step_kernel << <dimGrid, dimBlock >> > (state_out, state_init, dev_tend, dt, nx, hs, nz);

    cudaDeviceSynchronize();
    // cudaMemcpy(state_out, dev_state, (nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS * sizeof(real), cudaMemcpyDeviceToHost);
}

__global__ void compute_x_flux_kernel(real* state, real* flux, real* hy_dens_cell, real* hy_dens_theta_cell, real hv_coef, int nx, int hs, int nz) {
    ...
    if (k < nz) {
        ...
        real r, u, w, t, p, stencil[4], d3_vals[NUM_VARS], vals[NUM_VARS];
...
void compute_tendencies_x(real* dev_state, real* flux, real* tend, real dt) {
    real hv_coef;
    //Compute the hyperviscosity coefficient
    hv_coef = -hv_beta * dx / (16 * dt);

    // cudaMemcpy(dev_state, state, (nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS * sizeof(real), cudaMemcpyHostToDevice);

    dim3 dimGrid((nz) * (nx + 1) / 128 + 1);
    dim3 dimBlock(128);

    compute_x_flux_kernel << <dimGrid, dimBlock >> > (dev_state, dev_flux, dev_hy_dens_cell, dev_hy_dens_theta_cell, hv_coef, nx, hs, nz);

    cudaDeviceSynchronize();

    dim3 dimGrid_1((nz) * (nx) / 32 + 1);
    dim3 dimBlock_1(128);
    compute_x_tend_kernel << <dimGrid_1, dimBlock_1 >> > (dev_tend, dev_flux, nz, nx, dx);

    cudaDeviceSynchronize();

    cudaMemcpy(tend, dev_tend, nx * nz * NUM_VARS * sizeof(real), cudaMemcpyDeviceToHost);
}

__global__ void compute_z_flux_kernel(real* state, real* flux, real* hy_dens_int, real* hy_dens_theta_int, real* hy_pressure_int, real hv_coef, int nx, int hs, int nz) {
    int index = blockIdx.x * 128 + threadIdx.x;
    int k = index / nx;
    int i = index % nx;
    if (k < nz + 1) {
...
__global__ void compute_z_tend_kernel(real* tend, real* flux, real* state, int nx, int hs, int nz, real grav, real dz) {
    int indf1, indf2, indt, inds;
    int index = blockIdx.x * 128 + threadIdx.x;
    int k = (index / 4) / nx;
    int i = (index / 4) % nx;
    int ll = index % 4;
...
//Then, compute the tendencies using those fluxes
void compute_tendencies_z(real* dev_state, real* flux, real* tend, real dt) {
    real hv_coef;
    //Compute the hyperviscosity coefficient
    ...
    dim3 dimGrid((nz + 1) * nx / 128 + 1);
    dim3 dimBlock(128);

    compute_z_flux_kernel << <dimGrid, dimBlock >> > (dev_state, dev_flux, dev_hy_dens_int, dev_hy_dens_theta_int, dev_hy_pressure_int, hv_coef, nx, hs, nz);

    cudaDeviceSynchronize();

    cudaMemcpy(flux, dev_flux, (nx + 1) * (nz + 1) * NUM_VARS * sizeof(real), cudaMemcpyDeviceToHost);

    dim3 dimGrid_1(nz * nx / 32 + 1);
    dim3 dimBlock_1(128);
    compute_z_tend_kernel << <dimGrid_1, dimBlock_1 >> > (dev_tend, dev_flux, dev_state, nx, hs, nz, grav, dz);
    /////////////////////////////////////////////////
    // TODO: THREAD ME
    /////////////////////////////////////////////////
...
__global__ void set_halo_values_x_kernel(real* state, int nz, int hs, int nx) {
    int k = blockIdx.x * 32 + threadIdx.x / 4;
...
void set_halo_values_x(real* dev_state) {
    // cudaMemcpy(dev_state, state, (nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS * sizeof(real), cudaMemcpyHostToDevice);

    dim3 dimGrid(nz / 32 + 1);
    dim3 dimBlock(128);

    set_halo_values_x_kernel << <dimGrid, dimBlock >> > (dev_state, nz, hs, nx);
}

__global__ void set_halo_values_z_kernel(real* state, real* hy_dens_cell, int nz, int hs, int nx) {
    int i = blockIdx.x * 32 + threadIdx.x / 4;
    int ll = threadIdx.x % 4;
    if (i < nx + 2 * hs) {
        if (ll == ID_WMOM) {
...
void set_halo_values_z(real* dev_state) {
    // cudaMemcpy(dev_state, state, (nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS * sizeof(real), cudaMemcpyHostToDevice);
    dim3 dimGrid((nx + 2 * hs) / 32 + 1);
    dim3 dimBlock(128);

    set_halo_values_z_kernel << <dimGrid, dimBlock >> > (dev_state, dev_hy_dens_cell, nz, hs, nx);
...
void init(int* argc, char*** argv) {
    ...
    cudaMemcpy(dev_hy_dens_cell, hy_dens_cell, (nz + 2 * hs) * sizeof(real), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_hy_dens_theta_cell, hy_dens_theta_cell, (nz + 2 * hs) * sizeof(real), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_hy_dens_int, hy_dens_int, (nz + 1) * sizeof(real), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_hy_dens_theta_int, hy_dens_theta_int, (nz + 1) * sizeof(real), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_hy_pressure_int, hy_pressure_int, (nz + 1) * sizeof(real), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_state, state, (nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS * sizeof(real), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_state_tmp, state_tmp, (nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS * sizeof(real), cudaMemcpyHostToDevice);
}
...
void output(real* dev_state, double etime) {
    cudaMemcpy(state, dev_state, (nx + 2 * hs) * (nz + 2 * hs) * NUM_VARS * sizeof(real), cudaMemcpyDeviceToHost);
...
void finalize() {
    ...
    cudaFree(dev_state);
    cudaFree(dev_state_tmp);
    cudaFree(dev_flux);
    cudaFree(dev_tend);
    cudaFree(dev_hy_dens_cell);
    cudaFree(dev_hy_dens_theta_cell);
    cudaFree(dev_hy_dens_int);
    cudaFree(dev_hy_dens_theta_int);
    cudaFree(dev_hy_pressure_int);
    ierr = MPI_Finalize();
}
